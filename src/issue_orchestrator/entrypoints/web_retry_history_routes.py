...
async def get_history(orchestrator: WebOrchestratorDependency) -> JSONResponse:
    ...
    entries = []
    for entry in latest_history_entries_by_issue(orchestrator.state.session_history, limit=50):
        entries.append({
...
async def retry_publish_issue(
    ...
    result = orchestrator.deps.publish_recovery.retry_publish(issue_number, orchestrator.state)
