...
_TERMINAL_EVENTS = frozenset({"issue.blocked", "issue.needs_human", "issue.completed"})
...
def enrich_logical_semantics(
    *,
    ...
    restart_due_to_transition = (
        previous_event_name in _TERMINAL_EVENTS and event_name in _ITERATION_START_EVENTS
    )
