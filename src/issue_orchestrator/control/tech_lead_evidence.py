"""Owner for the tech_lead EVIDENCE MAP — a god-view location manifest + warm-cache.

The tech lead (ADR-0031) receives ``board-snapshot.json`` today. That
...
outcomes, the tech_lead case-file ledger, and anything added later), local ``git``
...
``git``. The call-site wrapper (``tech_lead_session_policy._stage_evidence_map``)
...
# Canonical evidence-map filename inside a session's tech-lead-data directory,
...
# (timeline/tech_lead_authority/label_store/queue_cache/session_registry/… — see
...
_DB_DESCRIPTION_HINTS = {
    ...
    "tech_lead_authority": "tech_lead case-file / pattern / shipped-fix ledger",
}
...
_GUIDANCE = (
    ...
    "tech_lead_authority=case-file/pattern/shipped-fix ledger, plus any store added "
...
@dataclass(frozen=True)
class EvidenceMap:
    """A tech_lead session's read-side evidence manifest (schema_version 2).
...
def _resolve_default_branch(
    ...
    try:
        ...
        logger.warning(
            "[tech_lead] Evidence-map default-branch resolution failed; leaving it "
...
def _discover_sqlite_locations(state_dir_path: Path) -> list[EvidenceLocation]:
    ...
    (its parent, where ``e2e.db`` lives), so timeline + tech_lead_authority + e2e +
    ...
    for root in (state_dir_path, data_root):
        for pattern in _DB_SUFFIX_GLOBS:
            try:
                ...
                logger.warning(
                    "[tech_lead] Evidence-map SQLite glob %s in %s failed: %s",
                    pattern,
                    root,
                    exc,
                )
...
def _registered_managed_worktrees(config: "Config") -> list[Path]:
    ...
    try:
        ...
        logger.warning(
            "[tech_lead] Evidence-map worktree glob in %s failed: %s", worktrees_root, exc
        )
        return []
    ...
    if len(registered) > _MAX_WORKTREE_ROOTS:
        logger.warning(
            "[tech_lead] Evidence-map managed worktrees truncated to %d (newest kept)",
            _MAX_WORKTREE_ROOTS,
        )
...
def _bounded_run_dirs(found: set[str]) -> tuple[str, ...]:
    ...
    logger.warning(
        "[tech_lead] Evidence-map run-dirs truncated to %d of %d (newest kept)",
        _MAX_RUN_DIRS,
        len(found),
    )
...
def _resolve_run_dirs(
    ...
    """Absolute, de-duped, sorted, bounded run-dir paths for this tech_lead session.
    ...
    try:
        ...
        logger.warning(
            "[tech_lead] Evidence-map run-dir discovery failed (partial result): %s",
            exc,
        )
...
def _build_github_warm_cache(
    ...
    try:
        ...
        logger.warning(
            "[tech_lead] Evidence-map GitHub warm-cache unavailable for issue #%s: %s",
            focus_issue_number,
            exc,
        )
        return None
...
def build_evidence_map(
    *,
    ...
    """Build the evidence map for a tech_lead session.
...
def write_evidence_map(run_dir: Path, evidence: EvidenceMap) -> Path:
    ...
    logger.info(
        "[tech_lead] Evidence map written: %s (focus=%s, %d location(s), %d run-dir(s))",
        path,
