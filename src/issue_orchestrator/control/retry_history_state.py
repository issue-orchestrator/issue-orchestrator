...
class RetryHistoryState:
    ...
    def prioritize_blocked_front(self, issue_number: int) -> bool:
        ...
        have a lifecycle. Records the issue in ``blocked_front_prioritized`` so
        ...
        if issue_number in self._state.priority_queue:
            return False
        self._state.priority_queue.insert(0, issue_number)
        self._state.blocked_front_prioritized.append(issue_number)
        return True

    def release_blocked_front(self, issue_number: int) -> bool:
        """Drop a blocked->front entry from the queue AND its ledger (#6873).

        The lifecycle counterpart to :meth:`prioritize_blocked_front`, called both
        on successful launch (via the always-run launch handler
        ``OrchestratorSupport._handle_launch_session``, not the tech-lead-specific
        ``ExpediteLane``) and on re-block / out-of-band unavailability (by the
        policy). Removes the issue from BOTH
        ``blocked_front_prioritized`` and ``priority_queue``. No-op — and never
        touches an operator/tech-lead entry — for an issue this lane doesn't own.
        """
        if issue_number not in self._state.blocked_front_prioritized:
            return False
        self._state.blocked_front_prioritized = [
            n for n in self._state.blocked_front_prioritized if n != issue_number
        ]
    ...
    def _prune_blocked_front_ledger(self) -> None:
        ...
        self._state.blocked_front_prioritized = [
            n for n in self._state.blocked_front_prioritized
            if n in self._state.priority_queue
        ]
