...
case-file lifecycle, mirroring ``triage_proposals`` (#6778) piece for piece:

* **Composition** — :func:`build_case_file_issue_action` turns the first
  observation of a signature into a :class:`CreateTriageCaseFileIssueAction`.
...
* **Creation boundary** — the applier's single create-issue executor
  (``triage_issue_creation.apply_create_triage_issue``) records the
...
* **Classification** — :func:`split_triage_case_file_issues` partitions the
  fact gatherer's ONE open-issue anchor scan (no extra GitHub call):
  observation-labeled issues become :class:`TriageCaseFileSummary` facts for
...
from ..domain.triage_session import (
    ...
    is_triage_observation_label,
    triage_area_from_labels,
)
from .actions import CreateTriageCaseFileIssueAction
from .triage_issue_policy import case_file_issue_labels

if TYPE_CHECKING:
    from ..domain.triage_artifacts import ProposedTriageAction, TriageFinding
...
def build_case_file_issue_action(
    ...
    *,
    ...
    body = (
        ...
        "A triage session flagged a recurring cross-job pattern. This issue"
        ...
        f" never picked up as agent work (`{TRIAGE_OBSERVATION_LABEL}`)."
    ...
    return CreateTriageCaseFileIssueAction(
        ...
        reason=(
            f"triage decision action {proposed.id}: open pattern case file"
...
def build_case_file_summary(issue: "Issue") -> TriageCaseFileSummary:
    ...
    return TriageCaseFileSummary(
        ...
        area=triage_area_from_labels(issue.labels),
    )


def split_triage_case_file_issues(
    issues: Sequence["Issue"],
) -> tuple[list["Issue"], tuple[TriageCaseFileSummary, ...]]:
    ...
    case_files: list[TriageCaseFileSummary] = []
    for issue in issues:
        if any(is_triage_observation_label(label) for label in issue.labels):
