...
class OrchestratorHttpApi(OrchestratorApi):
    def __init__(self, base_url_provider: Callable[[], str], client: httpx.Client) -> None:
    ...
    def refresh(self, inflight_stable_ids: list[str]) -> dict[str, Any]:
        return self._request("POST", "/api/refresh", json={"inflight_stable_ids": inflight_stable_ids})
    ...
    def send(self, issue_number: int, text: str) -> dict[str, Any]:
        return self._request("POST", f"/api/send/{issue_number}", json={"text": text})
