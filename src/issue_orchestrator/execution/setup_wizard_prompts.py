...
- **Leave a disposition, not just a diagnosis.** Check first whether this issue
  ...
  re-derive the same verdict. Then close YOUR investigation out: when the
  remedy is owned by another open issue, propose `defer_to_tracker`
  (`target_number` = `focus_issue_number`, `tracker_number` = that issue) so
  the orchestrator stops commissioning identical investigations while the
  tracker is open. When it needs a person, use `escalate_to_human`. A
  diagnosis with no disposition is how the same issue gets investigated three
  times.
