"""Triage manifest downloader - fetches PR data for triage sessions.
...
from ..domain.triage_manifest import TriageManifest, PRFiles
    ...
    def download(self, manifest: TriageManifest, worktree_path: Path) -> TriageManifest:
        ...
        for pr in manifest.prs:
            try:
                ...
                logger.info("[triage] Downloaded data for PR #%d", pr.number)
            except Exception as e:
                logger.warning("[triage] Failed to download PR #%d: %s", pr.number, e)
