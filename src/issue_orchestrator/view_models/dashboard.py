...
def _build_provider_circuit_breakers(orchestrator: Any) -> list[dict[str, Any]]:
    """Extract provider circuit breaker states from the orchestrator."""
    try:
        manager = orchestrator.deps.provider_resilience
        states = manager.store.list_all()
    ...
    for s in states:
        ...
        if s.open_until is not None:
            entry["open_until"] = s.open_until.isoformat()
            if is_open:
                remaining = (s.open_until - now).total_seconds()
                entry["cooldown_remaining_seconds"] = int(remaining)
        result.append(entry)
