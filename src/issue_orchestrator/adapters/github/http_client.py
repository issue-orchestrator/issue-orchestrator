...
def describe_github_token_sources(
    *,
    ...
    """
    repo_scoped_auth = any((configured_env, configured_keyring_service, configured_keyring_username))
    ...
    if configured_keyring_service or configured_keyring_username:
        ...
        if value:
            token_sources.append(f"Keyring ({keyring_service}/{keyring_username}): {_mask_token(value)}")
    if repo_scoped_auth:
            ...
            if repo:
                ...
                if repo_resp.status_code != 200:
                    return TokenValidationResult(
                        valid=False,
                        username=username,
                        error=f"Token cannot access repo {repo} (HTTP {repo_resp.status_code})",
                    )
            return TokenValidationResult(
                valid=True,
                username=username,
