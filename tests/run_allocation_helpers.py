...
def allocation_for(output, configuration=None):
    return IssueRunAllocationService(output, MemoryIssueRunLedger(), configuration=configuration if configuration is not None else Config(repo="example/repo"))
...
def make_session_launcher(*args, **kwargs) -> SessionLauncher:
    ...
    kwargs.setdefault("issue_run_allocator", IssueRunAllocationService(output, ledger, configuration=configuration))
