...
test('fresh lifecycle rerun requests return canonical endpoints and payloads', () => {
    const preflight = uiActionContract.buildFreshLifecycleRerunPreflightRequest([10, '11', 'x']);
    assert.equal(preflight.endpoint, '/api/reset-retry/fresh-lifecycle-rerun/preflight');
    assert.equal(preflight.method, 'POST');
    assert.deepEqual(preflight.body, { issues: [10, 11] });

    const execute = uiActionContract.buildFreshLifecycleRerunExecuteRequest([10, '11', 'x']);
    assert.equal(execute.endpoint, '/api/reset-retry/fresh-lifecycle-rerun');
