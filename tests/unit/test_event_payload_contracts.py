...
def test_queue_changed_event_payload_shape():
    ...
    emit_queue_changes(events, state, new_queue)  # type: ignore - Union type narrowing limitation
...
def test_stale_persistent_detected_payload_shape():
    ...
    track_stale_ticks(config, events, context, state, stale_issues=stale_issues)  # type: ignore - Union type narrowing limitation
