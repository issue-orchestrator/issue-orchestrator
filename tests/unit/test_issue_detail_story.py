...
class TestJourneySteps:
    ...
    def test_duplicate_detail_fragments_are_deduped(self):
        detail = "artifact missing: /tmp/x | artifact missing: /tmp/x"
        events = [_evt("session.blocked", timestamp="2026-02-09T20:15:00Z", detail=detail)]
        steps = _build_journey_steps(events, "2026-02-09")
        assert steps[0]["detail"] == "artifact missing: /tmp/x"
