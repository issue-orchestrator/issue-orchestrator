...
from issue_orchestrator.domain.models import AgentConfig, Issue, Session, SessionKey, TaskKind
...
def _session(tmp_path: Path, run_dir: Path | None) -> Session:
...
def test_recorded_run_dir_is_authoritative(tmp_path: Path) -> None:
    run_dir = tmp_path / "worktree" / ".issue-orchestrator" / "sessions" / "20260525__coding-1"
...
def test_legacy_session_uses_manifest_checked_fallback(tmp_path: Path) -> None:
    ...
    assert session_output.find_run_dir.call_args_list[1].args == (session.worktree_path,)
