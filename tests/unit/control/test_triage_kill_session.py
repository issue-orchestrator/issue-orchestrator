"""Tests for the triage kill_hung_session execution owner (#6778)."""
...
) -> tuple[TriageKillSessionExecutor, MagicMock, MagicMock]:
    events = MagicMock()
    run_kill = MagicMock(
        return_value=outcome or KillSessionRunOutcome(success=True)
    )
    executor = TriageKillSessionExecutor(
        events=events,
...
def test_executes_termination_and_publishes_executed_event() -> None:
    ...
    assert event.name == EventName.TRIAGE_ACTION_EXECUTED.value
...
def test_stale_downgrade_posts_no_mutations() -> None:
    ...
    [event] = [e.args[0] for e in events.publish.call_args_list]
    assert event.name == EventName.TRIAGE_ACTION_PROPOSED.value
