"""Tests for the untrusted triage artifact pair loader."""

import json
from pathlib import Path

from issue_orchestrator.control.triage_decision_loader import (
    TriageDecisionLoadFailure,
    load_triage_artifact_pair,
)
...
def _write_pair(
    ...
    *,
    ...
    decision_path = tmp_path / "triage-decision.json"
...
def test_missing_decision_file(tmp_path: Path) -> None:
    ...
    result = load_triage_artifact_pair(decision_path, report_path)

    assert not result.ok
    assert result.failure == TriageDecisionLoadFailure.MISSING_DECISION
    assert "triage-decision.json" in result.detail
...
def test_missing_report_file(tmp_path: Path) -> None:
    ...
    assert not result.ok
    assert result.failure == TriageDecisionLoadFailure.MISSING_REPORT
    assert "triage-report.md" in result.detail
...
def test_oversized_decision_file(tmp_path: Path) -> None:
    ...
    assert not result.ok
    assert result.failure == TriageDecisionLoadFailure.TOO_LARGE
    assert "triage-decision.json" in result.detail
...
def test_invalid_json(tmp_path: Path) -> None:
    ...
    assert not result.ok
    assert result.failure == TriageDecisionLoadFailure.INVALID_JSON
...
def test_contract_violation_bad_action_type(tmp_path: Path) -> None:
    ...
    assert not result.ok
    assert result.failure == TriageDecisionLoadFailure.CONTRACT_VIOLATION
