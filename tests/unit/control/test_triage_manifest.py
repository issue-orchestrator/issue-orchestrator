"""Unit tests for TriageManifest and related dataclasses."""
...
class TestTriageManifest:
    """Tests for TriageManifest."""

    def test_default_values(self):
        """TriageManifest defaults correctly."""
    ...
    def test_to_dict_empty(self):
        ...
        manifest = TriageManifest(data_dir=".issue-orchestrator/sessions/run1/triage-data")
        result = manifest.to_dict()

        assert result["session_type"] == "triage"
        assert result["data_dir"] == ".issue-orchestrator/sessions/run1/triage-data"
        assert result["prs"] == []
    ...
    def test_roundtrip(self):
        ...
        restored = TriageManifest.from_dict(as_dict)
    ...
    def test_write_creates_file(self, tmp_path: Path):
        ...
        manifest_path = tmp_path / "triage-data" / "manifest.json"
        manifest.write(manifest_path)

        assert manifest_path.exists()
        content = json.loads(manifest_path.read_text())
        assert content["session_type"] == "triage"
    ...
    def test_read_loads_file(self, tmp_path: Path):
        ...
        manifest = TriageManifest.read(manifest_path)
    ...
    def test_write_then_read_roundtrip(self, tmp_path: Path):
        ...
        original.write(path)
        loaded = TriageManifest.read(path)
