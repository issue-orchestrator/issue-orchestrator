...
class TestNoCacheAndInvalid:
    ...
    def test_invalid_when_status_known_but_reason_unknown(self) -> None:
        ...
        with pytest.raises(ValueError):
            _facts(reason="unrecognized_reason")

    def test_invalid_when_reason_known_but_status_unrecognized(self) -> None:
        with pytest.raises(ValueError):
            _facts(status="weird_status", reason=REASON_REVIEWER_OK)
    ...
    def test_invalid_when_pair_exists_but_doesnt_match_known_combos(self) -> None:
        ...
        with pytest.raises(ValueError):
            _facts(status="ok", reason=REASON_REVIEWER_NO_COMPLETION)
...
class TestReuseApproval:
    ...
    def test_ok_at_current_head_with_no_current_validation_when_not_required(
        self,
    ) -> None:
        assert (
            decide(
...
class TestReuseHalt:
    ...
    def test_stopped_outcomes_reuse_halt(self, reason: str) -> None:
        assert (
            decide(_facts(status="stopped", reason=reason)) is ResumeDecision.REUSE_HALT
        )
    ...
    def test_reviewer_decision_invalid_at_current_head_is_reuse_halt(self) -> None:
        assert (
            decide(
                _facts(
                    status="error",
                    reason=REASON_REVIEWER_DECISION_INVALID,
                )
            )
            is ResumeDecision.REUSE_HALT
        )
...
class TestCountNoCompletionAndRetry:
    ...
    def test_no_completion_at_current_head_is_count_and_retry(
        self, reason: str
    ) -> None:
        assert (
            decide(
...
_MATRIX_AT_CURRENT_HEAD: list[tuple[str, str, ResumeDecision]] = [
    ...
    (
        "error",
        REASON_REVIEWER_NO_COMPLETION,
        ResumeDecision.COUNT_NO_COMPLETION_AND_RETRY,
    ),
    ...
    ("error", REASON_REVIEWER_DECISION_INVALID, ResumeDecision.REUSE_HALT),
]
...
def test_matrix_covers_every_known_pair_in_decide() -> None:
    ...
    matrix_pairs = {(str(s), str(r)) for s, r, _ in _MATRIX_AT_CURRENT_HEAD}
    known_pairs = {
        (str(status), str(reason))
        for status, reason in rer._KNOWN_STATUS_REASON_PAIRS  # noqa: SLF001
    }
