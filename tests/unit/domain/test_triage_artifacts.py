"""Unit tests for the triage artifact pair contract (domain/triage_artifacts.py)."""
...
class TestTriageDecisionParsing:
    def test_round_trip(self):
        decision = TriageDecision.from_agent_payload(_payload())
        again = TriageDecision.from_agent_payload(decision.to_dict())
    ...
    def test_accepts_nested_decision_key(self):
        decision = TriageDecision.from_agent_payload({"decision": _payload()})
    ...
    def test_rejects_non_object_payload(self, bad):
        with pytest.raises(ValueError, match="JSON object"):
            TriageDecision.from_agent_payload(bad)

    def test_rejects_unknown_schema_version(self):
        with pytest.raises(ValueError, match="schema_version"):
            TriageDecision.from_agent_payload(_payload(schema_version=2))

    def test_rejects_missing_summary(self):
        payload = _payload()
        del payload["summary"]
        with pytest.raises(ValueError, match="summary"):
            TriageDecision.from_agent_payload(payload)

    def test_rejects_non_list_findings(self):
        with pytest.raises(ValueError, match="findings must be a list"):
            TriageDecision.from_agent_payload(_payload(findings={"id": "T1"}))

    def test_rejects_too_many_findings(self):
        findings = [_finding(f"T{n}") for n in range(MAX_TRIAGE_FINDINGS + 1)]
        with pytest.raises(ValueError, match="max"):
            TriageDecision.from_agent_payload(_payload(findings=findings))

    def test_rejects_too_many_actions(self):
        actions = [_action(f"A{n}") for n in range(MAX_TRIAGE_ACTIONS + 1)]
        with pytest.raises(ValueError, match="max"):
...
class TestTriageFindingParsing:
    ...
    def test_rejects_non_object_finding(self):
        with pytest.raises(ValueError, match="must be an object"):
            TriageDecision.from_agent_payload(_payload(findings=["just a string"]))

    def test_rejects_missing_title(self):
        finding = _finding()
        del finding["title"]
        with pytest.raises(ValueError, match="title"):
            TriageDecision.from_agent_payload(_payload(findings=[finding]))
...
class TestProposedActionParsing:
    ...
    def test_act_level_requires_target_and_rationale(self, act_type):
        ...
        parsed = TriageDecision.from_agent_payload(_payload(proposed_actions=[action]))
        assert parsed.proposed_actions[0].is_act_level
        broken = _action(action_type=act_type)
        del broken["body"]
        with pytest.raises(ValueError, match="rationale"):
            TriageDecision.from_agent_payload(_payload(proposed_actions=[broken]))

    def test_act_level_registry_is_subset_of_vocabulary(self):
        assert ACT_LEVEL_TRIAGE_ACTIONS < VALID_TRIAGE_ACTION_TYPES
...
class TestReportLinkValidation:
    ...
    def test_raises_listing_missing_ids(self):
        ...
        with pytest.raises(ValueError, match="T1"):
            validate_triage_report_links(decision, "# Report with no ids")
...
class TestEvidenceRequired:
    ...
    def test_direct_construction_cannot_bypass_evidence(self):
        decision = TriageDecision(
            summary="s",
            findings=(
                TriageFinding(id="T1", title="t", classification="infra"),
            ),
        )
...
class TestCanonicalIds:
    ...
    def test_non_canonical_finding_id_rejected(self, bad_id):
        with pytest.raises(ValueError, match="not canonical"):
            TriageDecision.from_agent_payload(_payload(findings=[_finding(bad_id)]))
    ...
    def test_combined_namespace_duplicates_rejected(self):
        ...
        from issue_orchestrator.domain import triage_artifacts
        ...
        original = triage_artifacts._ACTION_ID_RE
        triage_artifacts._ACTION_ID_RE = triage_artifacts._FINDING_ID_RE
        try:
            with pytest.raises(ValueError, match="share a namespace"):
                decision.validate()
        finally:
            triage_artifacts._ACTION_ID_RE = original
    ...
    def test_report_token_match_is_exact_not_substring(self):
        ...
        with pytest.raises(ValueError, match="exact token: T1(,|$)"):
            validate_triage_report_links(decision, "# Report\n\nT10 and A1 only.")
    ...
    def test_report_token_match_accepts_exact_tokens(self):
        ...
        decision = TriageDecision.from_agent_payload(payload)
        validate_triage_report_links(decision, "T1, T10 and A1 (see report).")


class TestDirectConstruction:
    def test_finding_from_mapping_rejects_evidence_non_list(self):
        with pytest.raises(ValueError, match="non-empty evidence list"):
            TriageFinding.from_mapping(_finding(evidence={"ref": 1}), index=1)

    def test_action_validate_is_idempotent_on_parsed(self):
        action = ProposedTriageAction.from_mapping(_action(), index=1)
        action.validate()
