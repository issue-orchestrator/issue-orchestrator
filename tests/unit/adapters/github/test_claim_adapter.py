...
class TestDetermineWinner:
    """Tests for _determine_winner method."""

    def test_higher_priority_wins(self, adapter):
        """Claim with higher priority wins."""
        now = datetime.now()
        claims = [
            Claim(
                lease_id="low-priority",
                claimant="orch-a",
                issue_number=42,
                started_at=now,
        ...
        winner = adapter._determine_winner(claims)

        assert winner is not None
        assert winner.lease_id == "high-priority"

    def test_lexicographic_lease_id_breaks_tie(self, adapter):
        """When priority is equal, larger lease_id wins."""
        now = datetime.now()
        claims = [
            Claim(
                lease_id="aaa-lease",
        ...
        assert winner is not None
        assert winner.lease_id == "zzz-lease"  # Lexicographically larger

    def test_expired_claims_ignored(self, adapter):
        """Expired claims are not considered."""
        now = datetime.now()
        claims = [
            Claim(
                lease_id="expired-claim",
                claimant="orch-a",
                issue_number=42,
                started_at=now - timedelta(hours=2),
                expires_at=now - timedelta(hours=1),  # Expired
                priority=9999,  # High priority but expired
            ),
            Claim(
                lease_id="valid-claim",
                claimant="orch-b",
                issue_number=42,
                started_at=now,
                expires_at=now + timedelta(hours=1),
        ...
        assert winner is not None
        assert winner.lease_id == "valid-claim"

    def test_returns_none_for_empty_list(self, adapter):
        """Returns None when no claims exist."""
        winner = adapter._determine_winner([])

        assert winner is None

    def test_returns_none_when_all_expired(self, adapter):
        """Returns None when all claims are expired."""
        now = datetime.now()
        claims = [
            Claim(
                lease_id="expired-1",
