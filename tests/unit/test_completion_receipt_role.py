...
def role_boundary(
    ...
    allocator = IssueRunAllocationService(output, ledger, branch_working_copy(), configuration=config)
...
def allocate(tmp_path, allocator, *, label="agent:tech-lead", exchange=False):
    ...
    return allocator.allocate(
        IssueRunAllocation(worktree, "issue-42", 42, key, label, "test", terminal_id="issue-42")
    )
