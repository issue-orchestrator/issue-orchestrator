...
class TestDeriveRunStatus:
    """Tests for the _derive_run_status helper method."""
    ...
    def test_completed_run_returns_completed(self, session_output, tmp_path):
        """Verify completed runs return 'completed' status."""
        manifest = {
            "ended_at": "2026-01-17T10:30:00Z",
            "outcome": "completed",
        }
        status = session_output._derive_run_status(manifest, tmp_path)
        assert status == "completed"

    def test_blocked_run_returns_blocked(self, session_output, tmp_path):
        """Verify blocked runs return 'blocked' status."""
        manifest = {
            "ended_at": "2026-01-17T10:30:00Z",
            "outcome": "blocked",
        }
    ...
    def test_timeout_run_returns_timeout(self, session_output, tmp_path):
        """Verify timed out runs return 'timeout' status."""
        manifest = {
            "ended_at": "2026-01-17T10:30:00Z",
            "outcome": "timeout",
        }
    ...
    def test_completed_with_validation_failed_returns_validation_failed(
        self, session_output, tmp_path
    ):
        """Verify runs with failed validation return 'validation_failed' status."""
        manifest = {
            "ended_at": "2026-01-17T10:30:00Z",
            "outcome": "completed",
    ...
    def test_no_ended_at_returns_in_progress(self, session_output, tmp_path):
        """Verify runs without ended_at return 'in_progress' status."""
        manifest = {
            "started_at": "2026-01-17T10:00:00Z",
        }
        status = session_output._derive_run_status(manifest, tmp_path)
