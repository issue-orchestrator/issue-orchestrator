    ...
    with (
        patch(
            ...
            return_value=[
                (
                    1234,
                    "python -m issue_orchestrator.entrypoints.tray --dashboard-url http://127.0.0.1:19080/",
                ),
                (
                    5678,
                    "python -m issue_orchestrator.entrypoints.tray --dashboard-url http://127.0.0.1:29080/",
                ),
            ],
        ),
        patch(
            "issue_orchestrator.entrypoints.control_center.os.getpid", return_value=9999
        ),
...
def test_start_tray_icon_starts_helper_process_on_macos() -> None:
    ...
    with (
        ...
        patch(
            "issue_orchestrator.entrypoints.control_center._cleanup_stale_tray_helpers"
        ),
        patch(
...
def test_main_dev_no_auth_clears_stale_auth_state(
    ...
    monkeypatch.setenv("ISSUE_ORCHESTRATOR_AGENT_CALLBACK_TOKEN", "env-stale-agent")
    ...
    with (
        ...
        patch(
            "issue_orchestrator.infra.repo_registry.cleanup_stale_repos", return_value=0
        ),
        patch(
            "issue_orchestrator.entrypoints.control_center.threading.Thread"
        ) as thread_cls,
        caplog.at_level(
            _logging.ERROR, logger="issue_orchestrator.entrypoints.control_center"
        ),
    ):
    ...
    assert any(
        "authentication is DISABLED" in record.getMessage() for record in caplog.records
    )
...
def test_main_dev_no_auth_refuses_non_loopback_host() -> None:
    ...
    with (
        ...
        patch(
            "issue_orchestrator.entrypoints.control_center.logging.basicConfig"
        ) as basic_cfg_mock,
        patch(
            "issue_orchestrator.infra.repo_registry.cleanup_stale_repos", return_value=0
        ) as cleanup_mock,
...
class TestLoopbackHostHelper:
    ...
    @pytest.mark.parametrize(
        "host",
        ["0.0.0.0", "::", "192.168.1.5", "10.0.0.1", "example.com", "", "not-an-ip"],
    )
...
def test_main_disables_uvicorn_access_log_by_default() -> None:
    ...
    with (
        patch(
            "argparse.ArgumentParser.parse_args",
            return_value=_make_main_args(debug_http=False),
        ),
        patch("issue_orchestrator.entrypoints.control_center.uvicorn.run") as run_mock,
        patch("issue_orchestrator.entrypoints.control_center.write_dashboard_pid"),
        patch("issue_orchestrator.entrypoints.control_center.clear_dashboard_pid"),
...
def test_main_enables_uvicorn_access_log_with_debug_http_flag() -> None:
    ...
    with (
        patch(
            "argparse.ArgumentParser.parse_args",
            return_value=_make_main_args(debug_http=True),
        ),
        patch("issue_orchestrator.entrypoints.control_center.uvicorn.run") as run_mock,
        patch("issue_orchestrator.entrypoints.control_center.write_dashboard_pid"),
        patch("issue_orchestrator.entrypoints.control_center.clear_dashboard_pid"),
...
def test_start_buttons_are_disabled_while_start_is_pending() -> None:
    ...
    template = (source_root / "templates" / "control_center.html").read_text(
        encoding="utf-8"
    )
    script = (source_root / "static" / "js" / "control_center.js").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        '<script src="/static/js/browser_auth.js?v={{ static_version }}"></script>'
        in template
    )
    assert (
        '<script src="/static/js/control_center_recovery_stop.js?v={{ static_version }}"></script>'
        in template
    )
    assert (
        '<script src="/static/js/control_center_recovery.js?v={{ static_version }}"></script>'
        in template
    )
    assert (
        '<script src="/static/js/control_center.js?v={{ static_version }}"></script>'
        in template
    )
    assert template.index("browser_auth.js") < template.index(
        "control_center_recovery_stop.js"
    )
    assert template.index("control_center_recovery_stop.js") < template.index(
        "control_center_recovery.js"
    )
    assert template.index("control_center_recovery.js") < template.index(
        "control_center.js"
    )
    assert 'id="recoveryStopDialog"' in template
    assert 'id="recoveryStopReason"' in template
    assert 'id="confirmRecoveryStop">Stop engine</button>' in template
    ...
    assert "getRecoveryView()?.invalidate(repoKey);" in script
    assert "navigateToEngineControls: focusRecoveryEngineControls" in script
    assert 'data-repo-key="${escapeHtml(repo.repo_key)}"' in script
    assert "controls?.querySelector('[data-action=\"stop\"]')" in script
    assert (
        "document.getElementById('sidebarCloseCC').addEventListener('click'" in script
    )
    ...
    assert (
        "Guardrails repaired (${fileCount} file${fileCount === 1 ? '' : 's'} written)"
        in script
    )
...
def test_recovery_stop_explanation_has_text_contrast_in_both_themes() -> None:
    css = (
    ...
    dark = re.search(r":root\s*\{(?P<body>.*?)\n\}", css, re.DOTALL)
    light = re.search(r'\[data-theme="light"\]\s*\{(?P<body>.*?)\n\}', css, re.DOTALL)
    ...
    for theme in (dark.group("body"), light.group("body")):
        assert (
            _contrast_ratio(token(theme, "text-secondary"), token(theme, "bg-card"))
            >= 4.5
        )
    stop_note_bodies = re.findall(
        r"\.repo-recovery-stop-note\s*\{(.*?)\}", css, re.DOTALL
    )
    assert any("color: var(--text-secondary);" in body for body in stop_note_bodies)
