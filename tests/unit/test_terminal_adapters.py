...
- ITermPlugin (execution/terminal_iterm.py)
- PluggySessionRunner (execution/session_runner_adapter.py)

These tests mock external dependencies (libtmux, subprocess/AppleScript calls)
...
class TestITermPlugin:
    """Tests for ITermPlugin which wraps ITermSessionManager.

    The plugin delegates to ITermSessionManager, so we mock the manager
    to test the plugin's hookimpl logic.
    """

    @pytest.fixture
    def mock_iterm_manager(self):
        """Mock ITermSessionManager for testing plugin delegation."""
        with patch("issue_orchestrator.execution.terminal_iterm.ITermSessionManager") as MockManager:
            mock_manager = MagicMock()
            MockManager.return_value = mock_manager
            yield mock_manager

    @pytest.fixture
    def iterm_plugin(self, mock_iterm_manager):
        """Create ITermPlugin with mocked manager."""
        from issue_orchestrator.execution.terminal_iterm import ITermPlugin
        return ITermPlugin()

    def test_create_session_success(self, iterm_plugin, mock_iterm_manager):
        """create_session delegates to manager and returns result."""
        mock_iterm_manager.create_session.return_value = True

        result = iterm_plugin.create_session(
            session_id=42,
            command="claude --prompt issue.md",
            working_dir="/tmp/worktree",
            title="Fix the bug",
        ...
        assert result is True
        mock_iterm_manager.create_session.assert_called_once_with(
            issue_number=42,
            command="claude --prompt issue.md",
            working_dir="/tmp/worktree",
            title="Fix the bug",
    ...
    def test_create_session_failure(self, iterm_plugin, mock_iterm_manager):
        """create_session returns False when manager fails."""
        mock_iterm_manager.create_session.return_value = False
    ...
    def test_session_exists_true(self, iterm_plugin, mock_iterm_manager):
        """session_exists returns True when tab exists and is running."""
        mock_iterm_manager.session_exists.return_value = True

        result = iterm_plugin.session_exists(session_id=42)

        assert result is True
        mock_iterm_manager.session_exists.assert_called_once_with(42)

    def test_session_exists_false(self, iterm_plugin, mock_iterm_manager):
        """session_exists returns False when tab doesn't exist or is idle."""
        mock_iterm_manager.session_exists.return_value = False

        result = iterm_plugin.session_exists(session_id=99)

        assert result is False

    def test_kill_session(self, iterm_plugin, mock_iterm_manager):
        """kill_session delegates to manager and returns result."""
        mock_iterm_manager.kill_session.return_value = True

        result = iterm_plugin.kill_session(session_id=42)

        assert result is True
        mock_iterm_manager.kill_session.assert_called_once_with(42)

    def test_discover_running_sessions(self, iterm_plugin):
        """discover_running_sessions calls discover_running_sessions function."""
        with patch("issue_orchestrator.execution.terminal_iterm.discover_running_sessions") as mock_discover:
            mock_discover.return_value = [
                {"issue_number": 42, "tab_name": "#42 Fix bug", "is_review": False},
                {"issue_number": 123, "tab_name": "#123", "is_review": True},
            ]

            result = iterm_plugin.discover_running_sessions()
    ...
    def test_cleanup_idle_sessions(self, iterm_plugin):
        """cleanup_idle_sessions calls cleanup_idle_tabs function."""
        with patch("issue_orchestrator.execution.terminal_iterm.cleanup_idle_tabs") as mock_cleanup:
            mock_cleanup.return_value = 3

            result = iterm_plugin.cleanup_idle_sessions()
    ...
    def test_send_to_session(self, iterm_plugin, mock_iterm_manager):
        ...
        mock_iterm_manager.send_to_session.return_value = True

        result = iterm_plugin.send_to_session(session_id=42, text="/exit")

        assert result is True
        mock_iterm_manager.send_to_session.assert_called_once_with(42, "/exit")
...
class TestITermSessionManagerIntegration:
    """Integration tests for ITermSessionManager with mocked AppleScript.

    These tests verify ITermSessionManager's logic while mocking subprocess
    calls to avoid needing a real iTerm2 instance.
    """

    @pytest.fixture
    def mock_applescript(self):
        """Mock run_applescript function."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            yield mock_run

    @pytest.fixture
    def mock_subprocess(self):
        """Mock subprocess.run for date command."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.subprocess.run") as mock_run:
    ...
    @pytest.fixture
    def iterm_manager(self):
        """Create ITermSessionManager."""
        from issue_orchestrator.adapters.terminal._iterm2 import ITermSessionManager
        return ITermSessionManager()

    def test_create_session_success(
        self, iterm_manager, mock_applescript, mock_subprocess, caplog
    ):
        """create_session creates iTerm2 tab with proper command."""
        ...
        with caplog.at_level(logging.INFO):
            result = iterm_manager.create_session(
                issue_number=42,
                command="claude --prompt issue.md",
                working_dir="/tmp/worktree",
                title="Fix the bug",
        ...
        assert result is True
        assert "Created iTerm2 tab for issue #42" in caplog.text

        # Verify 3 AppleScript calls were made
        assert mock_applescript.call_count == 3
        # The last call is the create tab script
        create_script = mock_applescript.call_args_list[2][0][0]
        assert "tell application \"iTerm\"" in create_script
        assert "create tab with default profile" in create_script
        assert "#42 Fix the bug" in create_script  # Tab name in escape sequence

    def test_create_session_failure(self, iterm_manager, mock_applescript, caplog):
        """create_session returns False when AppleScript fails."""
        ...
        assert result is False
        assert "Failed to create iTerm2 tab" in caplog.text

    def test_create_session_closes_idle_tabs_first(
        self, iterm_manager, mock_applescript, mock_subprocess
    ):
        ...
        assert result is True
        # Verify 3 AppleScript calls
        assert mock_applescript.call_count == 3

    def test_create_session_skips_if_running_exists(
        self, iterm_manager, mock_applescript, caplog
    ):
        ...
        assert result is False
        assert "active Claude session already exists" in caplog.text
    ...
    def test_kill_session_success(self, iterm_manager, mock_applescript):
        ...
        mock_applescript.return_value = (True, "true")
        iterm_manager._sessions[42] = {"tab_name": "#42"}

        result = iterm_manager.kill_session(42)

        assert result is True
        assert 42 not in iterm_manager._sessions

        # Verify AppleScript closes tab
        script = mock_applescript.call_args[0][0]
        assert "close t" in script

    def test_send_to_session_success(self, iterm_manager, mock_applescript):
        """send_to_session sends text to tab."""
        mock_applescript.return_value = (True, "true")

        result = iterm_manager.send_to_session(42, "/exit")

        assert result is True

        # Verify AppleScript writes text
        script = mock_applescript.call_args[0][0]
        assert 'write text "/exit"' in script
        assert "application id \"com.googlecode.iterm2\"" in script  # No focus stealing

    def test_send_to_session_escapes_quotes(self, iterm_manager, mock_applescript):
        """send_to_session escapes double quotes in text."""
        mock_applescript.return_value = (True, "true")

        iterm_manager.send_to_session(42, 'say "hello"')

        script = mock_applescript.call_args[0][0]
        assert 'write text "say \\"hello\\""' in script

    def test_list_sessions_filters_non_existent(self, iterm_manager, mock_applescript):
        """list_sessions only returns sessions that still exist."""
        iterm_manager._sessions = {42: {}, 99: {}, 123: {}}
        ...
        assert sorted(result) == [42, 123]


class TestDiscoverRunningSessions:
    """Tests for discover_running_sessions module function."""

    def test_discover_running_sessions_parses_output(self):
        """discover_running_sessions parses AppleScript output correctly."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            mock_run.return_value = (True, "#42 Fix bug, #123, #456 Review PR")

            from issue_orchestrator.adapters.terminal._iterm2 import discover_running_sessions

            result = discover_running_sessions()

            assert result == [
                {"issue_number": 42, "tab_name": "#42 Fix bug", "is_review": False},
                {"issue_number": 123, "tab_name": "#123", "is_review": False},
                {"issue_number": 456, "tab_name": "#456 Review PR", "is_review": True},
            ]

    def test_discover_running_sessions_handles_failure(self, caplog):
        """discover_running_sessions returns empty list on failure."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            ...
            assert result == []
            assert "Failed to discover running sessions" in caplog.text


class TestCleanupIdleTabs:
    """Tests for cleanup_idle_tabs module function."""

    def test_cleanup_idle_tabs_returns_count(self, caplog):
        """cleanup_idle_tabs returns number of tabs closed."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            mock_run.return_value = (True, "5")

            from issue_orchestrator.adapters.terminal._iterm2 import cleanup_idle_tabs

            with caplog.at_level(logging.INFO):
                result = cleanup_idle_tabs()

            assert result == 5
            assert "Closed 5 idle iTerm2 tabs" in caplog.text

    def test_cleanup_idle_tabs_handles_failure(self, caplog):
        """cleanup_idle_tabs returns 0 on failure."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            mock_run.return_value = (False, "Error")
            ...
            assert result == 0
            assert "Failed to cleanup idle tabs" in caplog.text

    def test_cleanup_idle_tabs_handles_non_numeric_output(self, caplog):
        """cleanup_idle_tabs handles unexpected output gracefully."""
        with patch("issue_orchestrator.adapters.terminal._iterm2.run_applescript") as mock_run:
            mock_run.return_value = (True, "not a number")
