...
class MockPRTracker:
    def __init__(self, prs: list[PRInfo]):
        self.prs = prs
...
class TestOrphanedPRLabelReconciliation:
    def test_reconcile_skips_out_of_scope_orphaned_pr(self):
        labels = MockLabelSet()
        pr = PRInfo(
            number=42,
            title="Out of scope",
            url="https://example.test/pull/42",
            branch="42-out-of-scope",
        ...
        sync = LabelSync(
            ...
            pr_tracker=MockPRTracker([pr]),
        )

        fixed_count = sync.reconcile_orphaned_pr_labels(
            "needs-code-review",
            "code-reviewed",
            "Generated by issue-orchestrator agent",
            is_pr_in_scope=lambda _: False,
        )

        assert fixed_count == 0
        assert labels.add_calls == []

    def test_reconcile_labels_in_scope_orphaned_pr(self):
        labels = MockLabelSet()
        pr = PRInfo(
            number=42,
            title="In scope",
        ...
        fixed_count = sync.reconcile_orphaned_pr_labels(
            ...
            "code-reviewed",
            "Generated by issue-orchestrator agent",
            is_pr_in_scope=lambda _: True,
        )

        assert fixed_count == 1
        assert labels.add_calls == [(42, "needs-code-review")]

    def test_reconcile_does_not_scope_check_already_labeled_pr(self):
        labels = MockLabelSet()
        pr = PRInfo(
            number=42,
        ...
        def fail_if_called(_: PRInfo) -> bool:
            raise AssertionError("scope predicate should not run for already-labeled PRs")

        fixed_count = sync.reconcile_orphaned_pr_labels(
            "needs-code-review",
            "code-reviewed",
            "Generated by issue-orchestrator agent",
            is_pr_in_scope=fail_if_called,
        )
