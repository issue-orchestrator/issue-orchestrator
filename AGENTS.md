...
## Browser JSON Boundaries (Generated Validators)

JSON crossing into browser JavaScript from a UI boundary is validated at
runtime against the generated contract. See
[docs/architecture/browser-json-contracts.md](docs/architecture/browser-json-contracts.md).

**Coders** — for any browser JSON boundary work (DOM `data-*` payloads, inline
`<script type="application/json">` bootstraps, `fetch().json()` responses,
event/stream payloads):

- Parse through the shared readers in `static/js/ui_contract_json.js`
  (`fromDataset` / `fromInlineScript` / `fromResponse` / `fromEventData` /
  `fromValue`), never a bare `JSON.parse` or `response.json()`.
- Payload shape is owned by the generated validators
  (`static/js/ui-contracts.validators.js`, regenerated by
  `scripts/generate_ui_contracts.py`). Do not hand-write checks for `kind`,
  required fields, enum values, ID types/ranges, or forbidden extra fields.
- Keep **contextual** invariants in the UI owner abstraction that holds the
  context (e.g. a command's `run_id` matching the DOM row that dispatched it).
  A schema cannot know those.
- Fail closed: readers return `null` on rejection, having already reported one
  diagnostic. Return before mutating UI state or issuing a request.
- If a boundary has no contract, add the component to `docs/api/ui-openapi.json`
  and regenerate — or record it as an intentional exception in the inventory.

**Reviewers** — for UI JSON payload work, verify that payload-shape validation is
generated/contract-backed rather than hand-crafted, and that true contextual
invariants still live in owner abstractions. Treat a re-introduced hand-rolled
shape check (or a raw `JSON.parse` of a contract-covered payload) as
implementation-required. If none exist, state:
`Browser JSON contract review: no issues found.`
