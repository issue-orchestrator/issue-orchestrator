...
| `ui.browser_session.max` | integer | `1024` | Upper bound on the in-memory session table. Beyond this, the least-recently-used entries are evicted on new login. | `256`, `1024`, `4096` | Practically only relevant on long-running deployments with many operators. The cap protects against unbounded memory growth, not against brute force. |
