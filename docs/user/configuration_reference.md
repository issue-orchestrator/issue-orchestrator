...
| `triage.stuck_sweep.interval_minutes` | integer | `15` | Scan for stuck issues every N minutes | `15`, `30`, `60` | How often the tech-lead sweep scans open issues for terminal stuck state. Only runs when the sweep is enabled; each scan is a single bounded query. |
