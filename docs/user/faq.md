...
| **Reset & retry** | Tears down local **and** remote state before re-queueing: **deletes the worktree and its detailed session artifacts**, **deletes the issue's local and remote branches**, **removes all orchestrator-owned labels**, and **clears the saved label persistence and session/completed-today history**. It does **not** supersede open PRs or clear the compact Timeline facts — so an open PR is left behind with its branch deleted (orphaned), while file-backed Timeline actions disappear. | The attempt is a write-off and you want the branch rebuilt from base, with no open PR that needs closing cleanly. |
...
Timeline keeps compact history, not a replay copy of deleted session files. If a
session needs later forensic replay, copy its artifacts somewhere intentionally
before deleting the worktree; the orchestrator does not create a second archive.
