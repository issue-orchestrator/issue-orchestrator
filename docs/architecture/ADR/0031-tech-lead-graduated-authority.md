...
| `defer_to_tracker` | Terminal disposition of a completed failure investigation: park the diagnosed issue on the OPEN issue that owns its remedy, so the stuck sweep stops re-diagnosing it until that tracker closes (#6971) | `execute` (floor: cannot be disabled) |
